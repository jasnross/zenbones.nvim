local function get_output_name(name, ness)
	if ness == "default" then
		return name
	else
		return string.format("%s_%s", name, ness)
	end
end
		...
		run(
			...
			function(colors)
				...
				return vim.tbl_extend("keep", colors, {
					name = get_output_name(name, ness),
			...
			{ overwrite, string.format("extras/kitty/%s.conf", get_output_name(name, ness)) }
		)
